...
.. c:macro:: XEN_DOMCTL_CLAIM_MEMORY_GLOBAL
   ...
   Value for the :c:expr:`xen_memory_claim.node` field of a claim set
   entry to specify a global claim satisfied from any node, useful to
   specify a global claim satisfied from any node, useful when strict
   per-node placement is not required or as a fallback for memory that
   may be populated on any node.

   These claims are consumed on allocation only when the allocation node's
...
.. c:macro:: XEN_DOMCTL_CLAIM_MEMORY_LEGACY
   ...
   Value for the :c:expr:`xen_memory_claim.node` field of a claim set entry
   to invoke the legacy claim path with its legacy semantics for backward
   compatibility with existing callers of :ref:`XENMEM_claim_pages`.

   Supported by :c:expr:`XEN_DOMCTL_claim_memory` for backward compatibility, but
   not supported by the new claim sets path.
...
Claim sets installation is invoked via :c:expr:`XEN_DOMCTL_claim_memory` and
...
- The request contains exactly one global claim entry of the form
  :c:expr:`xen_memory_claim.node = XEN_DOMCTL_CLAIM_MEMORY_LEGACY`.
