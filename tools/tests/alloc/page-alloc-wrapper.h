...
/* Additional synthetic allocator state used by the test environment. */
                                                ...
                                                memory_claim_t *claim_set)
{
    ...
    /* Avoid verbose assertion logging while installing a claim set. */
    ...
    for ( unsigned int i = 0; i < nr_claims; i++ )
    {
        ...
        printf("  Claim %u: pages=%" PRIu64 ", target=%s\n", i, claim_set[i].pages,
               target_str);
    }
                                                   ...
                                                   const char *caller_func)
{
    ...
    /* Avoid verbose assertion logging while marking a page offline. */
    testcase_assert_verbose_assertions = false;
                                             ...
                                             const char *caller_func)
{
    ...
    /* Avoid verbose assertion logging while setting outstanding claims. */
    testcase_assert_verbose_assertions = false;
                                                   ...
                                                   const char *caller_func)
{
    ...
    /* Avoid verbose assertion logging while allocating domheap pages. */
    testcase_assert_verbose_assertions = false;
...
static void __used test_free_domheap(struct page_info *pg, unsigned int order,
                                     const char *caller_func)
{
    bool save_verbose_asserts = testcase_assert_verbose_assertions;
...
#define free_domheap_pages(pg, order) test_free_domheap(pg, order, __func__)
...
#endif /* TOOLS_TESTS_ALLOC_PAGE_ALLOC_WRAPPER_H */
