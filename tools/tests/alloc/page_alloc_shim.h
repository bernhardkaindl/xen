                           ...
                           const char *file, int line, const char *func)
{
    ...
    if ( lock == heap_lock_ptr )
        ...
        fprintf(stdout, "dom1->page_alloc_lock %s\n", action);
    else if ( domain_list && lock == &test_dummy_domain2.page_alloc_lock )
        fprintf(stdout, "dom2->page_alloc_lock %s\n", action);
    else
...
/*
 * Initial fake NUMA stubs so page_alloc.c can compile be compiled with
 * CONFIG_NUMA enabled to support unit testing NUMA-aware claims using
 * page_alloc.c, but not intended to e.g. allocate memory on specific
 * NUMA nodes using get_free_buddy().
 *
 * These are not intended to be complete or accurate for general use in
 * other test contexts or as a general-purpose shim for NUMA functionality.
 */
