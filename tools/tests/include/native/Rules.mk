...
# Rules for building and running native tests.
...
# Architectures to build (and run using Linux binfmt) when cross=y is passed
...
endef
$(foreach arch,$(TEST_ARCHS), $(eval $(call LOAD_ARCH_CFLAGS,$(arch))))
...
SRCARCH := $(call get-srcarch,$(XEN_TARGET_ARCH))
...
$(eval $(call native-test-cflags,COMMON_CFLAGS,))
COMMON_CFLAGS += -I$(srctree)/include -I$(srctree) -I$(XEN_ROOT)/tools
COMMON_CFLAGS += $(CFLAGS_xeninclude) $(CFLAGS_tests_native)

ifdef TARGETS # make -C tools/tests/claims TARGETS=host-claims
...
SRCARCH_$(1) := $$(call get-srcarch,$$(ARCH_$(1)))
TEST_SOURCES_$(1) := $$(TEST_SOURCES)
$(call ARCH_template_extra,$(1))
...
.PHONY: setup-$(1)-asm-macros
setup-$(1)-asm-macros:
	$(MAKE) -C $(XEN_ROOT)/xen -f arch/$$(SRCARCH_$(1))/Makefile \
		obj=arch/$$(SRCARCH_$(1)) src=arch/$$(SRCARCH_$(1)) include
