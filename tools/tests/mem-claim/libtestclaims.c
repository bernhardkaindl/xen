...
int lib_get_outstanding_claims_global(struct test_ctx *ctx,
                     ...
                     memory_claim_t *claims, const char *reason)
{
    ...
    if ( rc )
    {
        ...
        lib_get_outstanding_claims_global(ctx, &outstanding_pages);
                          ...
                          unsigned int node, unsigned int spare)
{
     ...
     */
    lib_get_outstanding_claims_global(ctx, &global_before);
    rc = lib_claim_memory(
        ctx, domid, 1, &(memory_claim_t){.pages = claim_pages, .target = node},
        ctx->result->params);
    if ( rc )
        return rc;
    lib_get_outstanding_claims_global(ctx, &global_after);
                                   ...
                                   const char *reason)
{
     ...
     */
    lib_get_outstanding_claims_global(ctx, &outstanding_claims);
    lib_get_global_free_pages(ctx, &free_pages);
                     ...
                     const struct test_case *test, struct test_result *result)
{
     ...
     */
    rc = lib_get_outstanding_claims_global(&ctx, &baseline_outstanding);
    if ( rc )
        goto out;
