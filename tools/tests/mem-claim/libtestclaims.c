...
int lib_claim_memory(struct test_ctx *ctx, uint32_t domid, uint32_t records,
                     xen_domctl_memory_claim_t *claim_set, const char *reason)
{
    lib_set_step(ctx, "%s", reason);
    rc = xc_domain_set_memory_claims(ctx->env->xch, domid, records, claim_set);
    if ( rc )
    {
...
int lib_release_all_claims(struct test_ctx *ctx, uint32_t domid)
{
    ...
    rc = xc_domain_set_memory_claims(ctx->env->xch, domid, records,
                                    &(xen_domctl_memory_claim_t){});
    if ( rc )
        return lib_fail(ctx, "xc_domain_set_memory_claims(pages=0) failed");
    return 0;
}
                          ...
                          unsigned int node, unsigned int spare)
{
    ...
    rc = lib_claim_memory(
        ctx, domid, 1,
        &(xen_domctl_memory_claim_t){ .pages = claim_pages, .target = node },
        ctx->result->params);
    if ( rc )
        return rc;
