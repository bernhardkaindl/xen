...
static void test_alloc_domheap_redeems_claims(int start_mfn)
{
    ...
    CHECK(outstanding_claims == 0, "claims consumed after allocations");
    CHECK(total_avail_pages == 1, "one free page after allocations");
    ...
    CHECK(ret == -ENOMEM, "dom1 claim +1 fails due to insufficient pages");

    /* Claim more than dom1's d->max_pages fails with EINVAL */
    ret = install_host_claims(dom1, dom1->max_pages + 1);
    CHECK(ret == -EINVAL, "dom1 claim fails due to exceeding max_pages");
    ...
    CHECK(outstanding_claims == 1, "dom2's claims still present");
    CHECK(total_avail_pages == 1, "one free page after failed alloc");
}
