...
static void test_claims_numa_install(int start_mfn)
{
    ...
    ASSERT(install == 0);
    CHECK(outstanding_claims == 6, "Expect 6 total claims after installation");
    CLAIMS(dom1, claim_set);

    ASSERT(alloc_domheap_pages(dom1, order0, MEMF_node(node0)));
    CHECK(outstanding_claims == 5, "Expect 5 total claims left after allocation");
    CHECK(total_avail_pages == 7, "Expect 7 free pages left after allocation");
    ...
    CHECK(outstanding_claims == 4, "Expect 4 total claims left after allocation");
    CHECK(total_avail_pages == 6, "Expect 6 free pages left after allocation");
    ...
    CHECK(outstanding_claims == 2, "Expect 2 total claims left after allocation");
    CHECK(total_avail_pages == 4, "Expect 4 free pages left after allocation");
    ...
    ASSERT(alloc_domheap_pages(dom1, order0, MEMF_node(node1)));
    CHECK(outstanding_claims == 1, "Expect 1 total claims left after allocation");
    CHECK(total_avail_pages == 3, "Expect 3 free pages left after allocation");
    claim_set2[0].pages--; /* The 2nd page is redeemed from host-wide claim */
    CLAIMS(dom1, claim_set2);

    /* An allocation on node 1 falls back to node 0 */
    ASSERT(alloc_domheap_pages(dom1, order0, MEMF_node(node1)));
    CHECK(outstanding_claims == 0, "Expect 0 total claims left after allocation");
    CHECK(total_avail_pages == 2, "Expect 2 free pages left after allocation");
